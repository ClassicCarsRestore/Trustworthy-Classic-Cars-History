...
url1="https://gui.classicschain.com:8393/api/Restorations/UpdateAndPhotos/Server3"
method1="PUT"
...
measure_request_time() {
    ...
    do 
        ...
        start_time=$(date +%s.%N)
        response1=$(curl -X "$method1" -s -F "stepId=$stepId" -F "newTitle=$title" -F "newDescription=$description" -F "file=@$file_path0" -F "file=@$file_path1" -F "file=@$file_path2" -F "file=@$file_path3" -F "file=@$file_path4" -H "Authorization: Bearer $auth_token" -H "Content-Type: multipart/form-data" "$url1")
        end_time=$(date +%s.%N)
