...
url1="https://gui.classicschain.com:8393/api/Restorations/Create/Server4"
method1="POST"

file_path0="../testDocs/rolls0.jpg"
file_path1="../testDocs/rolls1.jpg"
#file_path1="../testDocs/10mb.mp4"
...
measure_request_time() {
    ...
    do 
        ...
        response1=$(curl -X "$method1" -s -F "title=$title" -F "description=$description" -F "file=@$file_path0" -F "file=@$file_path1" -H "Authorization: Bearer $auth_token" -H "Content-Type: multipart/form-data" "$url1")
        end_time=$(date +%s.%N)
