...
url1="https://gui.classicschain.com:8393/api/Restorations/Create/Server2"
method1="POST"

file_path="./testDocs/download.jpg"
...
measure_request_time() {
    ...
    do 
        ...
        response1=$(curl -X "$method1" -s -F "title=$title" -F "description=$description" -H "Authorization: Bearer $auth_token" -H "Content-Type: multipart/form-data" "$url1")
        end_time=$(date +%s.%N)
