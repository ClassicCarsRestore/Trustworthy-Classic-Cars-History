...
function networkDown() {
  ...
  COMPOSE_ORG3_FILES="${COMPOSE_ORG3_BASE_FILES} ${COMPOSE_ORG3_COUCH_FILES} ${COMPOSE_ORG3_CA_FILES}"docker
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm compose_orderer.example.com compose_orderer2.example.com compose_orderer3.example.com compose_peer0.org1.example.com compose_peer0.org2.example.com
    #docker volume prune -f
    #Cleanup the chaincode containers
    clearContainers
    #Cleanup images
    removeUnwantedImages
    #ADDED: to remove volumes that are not used
