...
* @description CreateClassic issues a new asset to the world state with given details.
...
// UpdateClassic updates an existing classic in the world state with provided parameters.
func (s *SmartContract) UpdateClassic(ctx contractapi.TransactionContextInterface, make string, model string, year int, licencePlate string, country string, chassisNo string, engineNo string, ownerEmail string, priorOwners []string, restorations []RestorationStep, certifications []string, documents map[string]string) error {
	exists, err := s.ClassicExists(ctx, chassisNo)
	if err != nil {
		return err
	}
...
func (s *SmartContract) CreateRestorationStep(ctx contractapi.TransactionContextInterface, chassisNo string, title string, description string, photosIds []string, when string) (string, error) {
	...
	return string("The classic "+ chassisNo + " got a new restoratio step sucessfully."), nil
}
...
func (s *SmartContract) HasViewerAccess(ctx contractapi.TransactionContextInterface, chassisNo string, currentTime string) (bool, error){
	...
	if ok {
		parsedTime = parsedTime.Add(10 * time.Minute)
